...
export async function proposeTransaction({ethers}: HardhatRuntimeEnvironment, network: string, {authoringSafe, to, data}: Transaction): Promise<string> {
  ...
  const safeService = new SafeApiKit({ txServiceUrl: serviceUrlForNetwork(network), ethAdapter })
